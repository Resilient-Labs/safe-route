...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (window.serverData && window.serverData.mapCenter) {
    ...
    console.info('Server data not available, using fallback values');
    ...
    markers.forEach((marker) => {
      if (marker.lat && marker.lng) {
        ...
        L.marker([marker.lat, marker.lng], { icon: icon })
          .addTo(map)
          .bindPopup(
            marker.title || marker.description || 'No description available'
          );
      }
    });
  }
  ...
  map.on('click', function (e) {
    ...
    document.getElementById('address').value = `${lat.toFixed(
      6
    )}, ${lng.toFixed(6)}`;
    ...
    geocoder.geocode(
      ...
      function (results) {
        ...
        if (results && results.length > 0) {
          ...
          formData.append('city', result.city || result.county || result.state || 'Unknown');
          ...
          let icon;
          switch (formData.get('incidentType')) {
            case 'accessibility':
              icon = accessibilityIcon;
              break;
            case 'suspicious activity':
...
function isWithinUSBounds(lat, lng) {
  const continentalUS =
    lat >= 25.0 && lat <= 49.0 && lng >= -125.0 && lng <= -66.9;
      ...
      if (results && results.length > 0) {
        ...
        if (window.map) {
          ...
          window.searchMarker = L.marker([lat, lng], {
            ...
            .addTo(window.map)
            .bindPopup(`Searched: ${result.name}`);
        }
      } else {
